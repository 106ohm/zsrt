...
allocate( v(n), vEispack(n) )

allocate( xPlot(n), statPlot(n) )
...
call rgg(nm,n,Teispack,Seispack,alfr,alfi,beta,matz,Z,ierr)
...
do i=1,n
   ...
   if ( abs( beta(i) ) <= machinePrecision ) then
...
do i=1,n
   ...
   if ( abs(v(i)) <= 1.d-17 ) then
      statPlot(i) = -20
   else
      statPlot(i) = exponent(v(i))*log10(2.0)
   end if
