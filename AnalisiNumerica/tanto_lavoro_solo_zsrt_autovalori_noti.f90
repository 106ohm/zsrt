...
IER = PGBEG(0,'Toeplitz_error_8.ps/PS',1,1)
if (IER.ne.1) stop
...
!!$   end do
   ...
   do i=1,n
      vEispack(i)= 8.d0 + 4.d0*cos((i*pi)/(n+1)) 
   end do
   ...
   zPlot(k)=sum(v)/(max(1,size(v)))

!   yPlot(k)=maxError

   write(*,*)"errore massimo=", yPlot(k)
   write(*,*)"media errori=", zPlot(k)
...
maxTotError=0.0
do i=1,kMax
   if ( maxTotError*1.d0 < yPlot(i) ) then
      maxTotError=yPlot(i)
   end if
end do
