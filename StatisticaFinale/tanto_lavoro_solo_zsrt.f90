...
real, dimension(:), allocatable :: xPlot, yPlot, zPlot, tPlot, wPlot, vPlot
...
allocate( xPlot(kmax), yPlot(kMax), zPlot(kMax), tPlot(kMax), wPlot(kMax), vPlot(kMax) )
...
IER = PGBEG(0,'Tempi_con_riferimento_quadrato_13_matrici_complete.ps/PS',1,1)
if (IER.ne.1) stop
...
!!$   end if
!!$
   wPlot(i) = (i*2)*log(2.d0) - (9*2)*log(2.d0)
   zPlot(i) = (i*3)*log(2.d0) - (9*3)*log(2.d0)
!!$   tPlot(i) = (i*4)*log(2.d0)
!!$
   if ( maxTotError < zPlot(i) ) then
      maxTotError = zPlot(i)
   end if
!!$
!!$   if ( maxTotError < tPlot(i) ) then
!!$      maxTotError = tPlot(i)
!!$   end if
!!$
...
CALL PGPT(kMax, xPlot, vPlot, 7)
CALL PGLINE(kmax,xPlot,vPlot)
CALL PGLAB('exponent k for n=pow(2,k)', 'time (log scale)', '')
